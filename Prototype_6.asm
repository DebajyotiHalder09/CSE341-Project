...
pass_strength_medium db 0Dh,0Ah,"Password strength: Medium!$"
pass_strength_medium_message db 0Dh,0Ah,"- This is a machine-generated password with good randomness, but it is recommended to use at least 10 characters for better security.$"
...
attempts_left_msg db 0Dh,0Ah,"Wrong key! Attempts left: $"
too_many_attempts_msg db 0Dh,0Ah,"Too many incorrect attempts! Exiting program.$"
...
key_validation_loop:
    ...
    mov ah, 01h
    int 21h
    mov bh, al  ; Store input key in BH
    
    ; Get the correct key for this user
    mov al, bl  ; BL contains user index
    mov ah, 0
    mov si, offset keys
    add si, ax
    
    ; Compare entered key with correct key
    cmp bh, [si]
    je key_correct  ; If key is correct, proceed to decrypt
    
    ; Key is incorrect, handle attempt
    dec cx  ; Decrement attempts counter
    jz too_many_attempts  ; If no attempts left, exit program
    
    ; Display incorrect key message
    lea dx, incorrect_key_msg
    mov ah, 9
    int 21h
    
    ; Display attempts left message
    lea dx, attempts_left_msg
    mov ah, 9
    int 21h
    
    ; Display number of attempts left
    mov dl, cl  ; Get attempts left
    add dl, '0'  ; Convert to ASCII
    mov ah, 2
    int 21h
    
    line_gap
    jmp key_validation_loop  ; Try again

too_many_attempts:
    ; Display too many attempts message
    lea dx, too_many_attempts_msg
    mov ah, 9
    int 21h
...
key_correct:
    ; Key is correct, proceed to decrypt and display password
...
generated_password_weak:
    LEA DX, pass_strength_medium
    MOV AH, 09h
    INT 21h
    
    LEA DX, pass_strength_medium_message
    MOV AH, 09h
    INT 21h
